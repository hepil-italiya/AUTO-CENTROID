...
def draw_bounding_box_on_image(image,
  ...
  if thickness > 0:
    ...
    #calculate the hight of object
    '''distance1=(bottom-top)
    distance1=float(distance1)
    distance1=distance1*0.0264
    distance1 = "{:.2f}".format(distance1)'''

    '''draw.line([(left, top), (left, bottom), (right, bottom), (right, top),
               (left, top)],
              width=thickness,
              fill=color)'''
    draw.line([(mid,top),(mid,bottom)],
              width=thickness,
              fill=color)
  ...
  for display_str in display_str_list[::-1]:
    ...
    #Display Hight of object
    '''draw.rectangle(
        [(right, text_bottom - text_height - 2 * margin), (right + text_width,
                                                          text_bottom)],
        fill=color)
    draw.text(
        (right + margin, text_bottom - text_height - margin),
         str(distance1)+' cm',
        fill='black',
        font=font)
    text_bottom -= text_height - 2 * margin'''

def give_middel_point():
  return mid
